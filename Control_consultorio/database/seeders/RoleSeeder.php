...
class RoleSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $roleAdmin = Role::create(['name'   => 'Medico']);
        ...
        $permissions = collect([
            ...
            ['group' => 'Paciente',                          'name' => "system.paciente.list",                      'description' => "Permiso para acceder al listado de pacientes",                             'permisos' =>  collect(['all'])],
            ['group' => 'Paciente',                          'name' => "system.paciente.create",                    'description' => "Permiso para registrar nuevos pacientes",                     'permisos' =>  collect(['all'])],
            ['group' => 'Paciente',                          'name' => "system.paciente.edit",                      'description' => "Permiso para editar la información de pacientes",         'permisos' =>  collect(['all'])],
            ['group' => 'Paciente',                          'name' => "system.paciente.status",                    'description' => "Permiso para activar/suspender pacientes",                    'permisos' =>  collect(['all'])],
        ...
        $permissions->each(function ($permission) use ($roleSuperAdmin, $roleAdmin) {
            ...
            if ($collection->search('all') !== false) {
                ...
                if ($collection->search('Medico') !== false) {
                    array_push($groups);
                }
            }
